...
export const TaskCard: React.FC<TaskCardProps> = ({ task, onStatusChange, onTimeUpdate }) => {
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 rounded-lg border p-4 hover:shadow-sm transition-all duration-200 ${
      overdue ? 'border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-950' : 'border-gray-200 dark:border-gray-800'
    }`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-wrap gap-1">
          {task.tags.map((tag, index) => (
            <span key={index} className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full">
              {tag}
            </span>
          ))}
        </div>
