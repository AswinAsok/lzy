        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-2">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{repository.name}</h3>
              {repository.isPrivate && (
                <span className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full">Private</span>
              )}
            </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="text-right">
              <div className="text-sm font-medium text-gray-900 dark:text-white">{repository.assignee}</div>
      ...
      <div className="p-6 space-y-4">
        ...
        {repository.currentTask && (
          <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3">
            <div className="flex items-start space-x-2">
              ...
              <div>
                <div className="text-sm font-medium text-gray-900 dark:text-white">Current Task</div>
                <div className="text-sm text-gray-700 dark:text-gray-300">{repository.currentTask}</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Last Commit</span>
          </div>
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
            <div className="text-sm text-gray-900 dark:text-white mb-1">{repository.lastCommit.message}</div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">{repository.pullRequests.length} open</span>
          </div>
          
          {repository.pullRequests.length > 0 ? (
            <div className="space-y-2">
              {repository.pullRequests.slice(0, 3).map((pr) => (
                <div key={pr.id} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-gray-900 dark:text-white truncate">#{pr.id} {pr.title}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">by {pr.author}</div>
                  </div>
                  <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getPRStatusColor(pr.status)}`}>
                    {pr.status.replace('_', ' ')}
                  </span>
                </div>
              ))}
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400 text-center py-2">No open pull requests</div>
          )}
        </div>
      </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="text-sm font-medium text-gray-900 dark:text-white capitalize">
                {repository.buildStatus.status === 'success' ? 'Deploy Successful' : 
                 repository.buildStatus.status === 'failed' ? 'Deploy Failed' :
                 repository.buildStatus.status === 'running' ? 'Building...' : 'Queued'}
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400">
